  ...
  cardsData.map((card_item) => {
    const item = `
        <div class="carousel_cards in-left">
            <a href="${card_item.url}" target="_blank">
                <img class="cardBackgroundImage" src="${card_item.image}" alt="${card_item.title}" />
            </a>
            <div class="cardFooter">
                <span class="cardTitle" title="${card_item.name}">${card_item.name}</span>
            </div>
        </div>`;
    cards += item;
  });
