      ...
      for (let i = 0; i < response.length; i += 1) {
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {
          if (response[i].text != null) {
            ...
            if (response[i].text?.toLowerCase().includes("rate our service")) {
              const clearfix = `<div class="clearfix"></div>`;
              console.log("Implementing star system");
              const ratingContainer = document.createElement("div");
              ratingContainer.className = "botMsg";
              ratingContainer.style.marginTop = "10px";

              const messageAndStars = document.createElement("div");
              const messageText = document.createElement("div");
              messageText.innerText = "How would you rate your experience?";
              messageAndStars.appendChild(messageText);

              const starRatingElement = createStarRating((rating) => {
                send(`/give_rating{"rating": ${rating}}`);
              });

              messageAndStars.appendChild(starRatingElement);
              ratingContainer.appendChild(messageAndStars);

              $(".chats").append(
                `<img class="botAvatar" src="./static/img/sara_avatar.png"/>`
              );
              $(".chats").append(ratingContainer);
              $(".chats").append(clearfix);
